...
MarkerDrag::finished (GdkEvent* event, bool movement_occurred)
{
	if (!movement_occurred) {
		...
		*/
		...
		switch (op) {
			case Selection::Set:
				if (editing_context.get_selection().selected (_marker) && editing_context.get_selection().markers.size () > 1) {
					editing_context.get_selection().set (_marker);
					_selection_changed = true;
				}
				break;
		...
		switch (_operation) {
			...
			case CreateCDMarker: {
				...
				if (_operation == CreateSkipMarker) {
					...
					editing_context.begin_reversible_command (_("new skip marker"));
					editing_context.session ()->locations ()->next_available_name (rangename, _("unnamed"));
		...
		if (_operation == CreateTransportMarker) {
			...
			switch (editing_context.current_mouse_mode()) {
				case MouseObject:
					...
					editing_context.select_all_within (start, end, 0.0f, FLT_MAX, _editor.track_views, Selection::Set, false);
					break;
