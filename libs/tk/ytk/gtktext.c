...
#ifdef __APPLE__
	  /* On MacOS, use Cmd-<foo> not Ctrl-<foo> for all these keyboard driven edits. The Command key
	     is (stupidly) indicated by MOD2+META (not just MOD2 as was done later for GTK+3).
	   */
	  if ((event->state & (GDK_MOD2_MASK|GDK_META_MASK)) == (GDK_MOD2_MASK|GDK_META_MASK)) /* Command key pressed */
	    {
	      return_val = TRUE;
	      if ((key >= 'A') && (key <= 'Z'))
		key -= 'A' - 'a';
