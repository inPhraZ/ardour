...
static void ComboDropDownItem_signal_scroll_horizontal_callback(GtkListItem* self, GtkScrollType p0,gfloat p1,void* data)
{
  ...
  ComboDropDownItem* obj = dynamic_cast<ComboDropDownItem*>(Glib::ObjectBase::_get_current_wrapper((GObject*) self));
...
static void ComboDropDown_signal_select_child_callback(GtkList* self, GtkWidget* p0,void* data)
{
  ...
  ComboDropDown* obj = dynamic_cast<ComboDropDown*>(Glib::ObjectBase::_get_current_wrapper((GObject*) self));
  // Do not try to call a signal on a disassociated wrapper.
  if(obj)
  {
    try
...
{
  iterator i = begin();
  while (i != end() && ((GtkWidget*)i->gobj() != w.gobj()))
    ++i;
  return i;
}
